    ...
    public class ReportController : Controller
    {
        ...
        public IActionResult CreateStockReport()
        {
            ...
            var model = new StockReportViewModel
            {
                ...
                StockRecords = new List<StockReportRecord>()
            };
        ...
        public IActionResult CreateStockReport(StockReportViewModel model)
        {
            ...
            if (!string.IsNullOrEmpty(model.SelectedTransactionCriteria))
            {
                model.StockRecords = GetStockRecords(model.SelectedTransactionCriteria);
            }
            else
            {
                model.StockRecords = new List<StockReportRecord>();
            }
        ...
        public IActionResult GenerateReport(StockReportViewModel model)
        {
            ...
            var stockRecords = GetStockRecords(model.SelectedTransactionCriteria);
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 3).Value = "Received Quantity";
                worksheet.Cell(1, 4).Value = "Issued Quantity";
                ...
                for (int i = 0; i < stockRecords.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 3).Value = stockRecords[i].ReceivedQuantity;
                    worksheet.Cell(i + 2, 4).Value = stockRecords[i].IssuedQuantity;
                    worksheet.Cell(i + 2, 5).Value = stockRecords[i].Stock;
                }
        ...
        private List<StockReportRecord> GetStockRecords(string selectedCriteria)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string query = @"
                    ...
                    WHERE ti.TransactionCode = @TransactionCode AND ti.ItemName = @ItemName";
                SqlCommand cmd = new SqlCommand(query, connection);
